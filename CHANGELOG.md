...
- **`DerivationDAG` value-provenance layer — the "would it match no matter what?" axis (`benchmarks.o10`).** The O10 structural invariants and `cit_readout` check *where* a value sits and *whether* it matches its witness; neither can see *how* the value was obtained — and a value that was fitted, normalised, post-selected, or convention-pinned matches by construction, so a clean cit edge corroborates the choice, not the theory. Each `Node` now carries a `provenance` (DERIVED / MEASURED / DEFINITION / NORMALIZED / FITTED / POST_SELECTED / CONVENTION / ASSERTED; default inferred — WITNESS→MEASURED, else DERIVED, so existing DAGs read as fully derived and the constants DAG audits clean). Three lints *mark, never repair* what the rest of the DAG misses: `provenance_defects()` (a non-derived OUTPUT that passes cit = circular), `tautology_nodes()` (a value defined to equal its own premise — the `J:=δF` then "derive" `d⋆F=J` pattern), and `asserted_operators()` (an operator depended on but never constructed — the undefined-Hodge-star smuggle); `provenance_audit()` is the one-call summary, surfaced in both `python -m nwt_substrate.benchmarks.o10` readouts. This complements M. Wende's redundancy layer (which already covers inert/decorative structure via `cut_nodes`/`criticality_ranking`) and encodes the failure modes a Maxwell-from-D12RG derivation review and an internal occasion-inflation κ=6 / H0-grid-search audit both turned on. Value-preserving; suite green (1440).
...
- **`solitons.faddeev` — Faddeev-Skyrme (Hopf) soliton primitives (pure numpy).** The construct/measure layer behind NWT's L₃ Skyrme-Faddeev sector (Paper 16): the forward, diagnostic half of the soliton work, with the energy-minimising JAX/GPU relaxers kept in the separate engine (one-way dependency; these numpy versions double as the engine's test oracle). `n_from_Z` is the CP¹ → S² map `n^a = Z†σ^aZ/Z†Z`. `solid_angle_triangle` / `plaquette_area_form` are the **faithful Berg-Lüscher area form** — the signed spherical area an S² field sweeps per lattice plaquette (two Van Oosterom-Strackee triangles); summed over a closed surface it is exactly 2π·(integer), supplying the lattice topological barrier the naive same-index `∂ᵢn×∂ⱼn` product lacks (the naive density is only O(dx) and lets a discrete hopfion unwind at every resolution). `whitehead_hopf_charge` is the **field-theoretic** Whitehead Hopf charge Q_H = (1/16π²)∫A·B (B from the area form, curl A = B solved spectrally) — distinct from the algebraic carrier-knot `topology.hopf_charge(p, m) = p·m`. `rational_hopfion` is a **smooth, compact, in-basin hopfion initial condition** built as a composition of smooth maps (Battye-Sutcliffe / Hietarinta-Salo rational map): it avoids the analytic stereographic seed's 0/0 spinor singularity on the core ring and its 1/r² approach to vacuum, instead reaching the south-pole vacuum *exactly* beyond a finite tube radius (periodic-safe), with charge Q_H = n·m. `faddeev_energy` / `energy_density` (forward-difference E₂ — no Nyquist null space — plus the area-form E₄) and `soliton_size` (energy-weighted RMS radius + the E₂/E₄ virial split) are the diagnostics; `solitons.BoxGrid` is the shared periodic cubic grid (a numpy mirror of the engine's JAX `Grid`, matching its centered-coordinate and `fftfreq` wavenumber conventions). These primitives stabilised the first lattice hopfion in the project (Q_H holds at +0.998 where every prior seed unwound to Q→0; E converges as O(dx²); E ∝ √c₄ Derrick scaling to 0.02%). 15 tests (VOS octant = π/2, orientation/degeneracy; unit spinor + vacuum map; seed charge ≈ 1 and = n·m, compact-vacuum-at-corner, smoothness, w ≤ R guard; vacuum energy/charge = 0; energy-density ↔ energy and E₂+E₄ ↔ energy identities).
- **`topology.linking_invariants` — Gauss linking number and higher-order link diagnostics (pure numpy).** The numeric primitive behind NWT's "binding = Hopf linking of carrier knots" picture (Paper 20), measured directly on point-sampled closed curves (the kind `diagrams.torus_knot_curve` / `hopf_link_curves` return). `gauss_linking_number(a, b)` evaluates the Gauss double integral; `linking_matrix(curves)` gives the pairwise matrix (its `.sum()/2` is the total linking number — the deuteron is a single clasp lk=−1, the alpha is four trefoils with all six K₄ edges clasped, total +6). `link_deletion_test` separates Borromean from "anti-Borromean" links (delete one component and see whether the rest fall apart); `borromean_rings()` returns the reference Borromean link (three eccentric perpendicular ellipses, pairwise lk=0) as a sibling generator to `hopf_link_curves`. `milnor_indeterminacy(L)` returns the gcd of the pairwise linkings — the modulus in which the triple Milnor (Massey/Borromean) invariant is defined: 0 ⇒ the Borromean regime (μ̄ a genuine integer), 1 ⇒ a spatial Massey invariant is *obstructed* (e.g. the alpha's all-+1 links). `tetrahedral_rotations()` + `link_symmetry_permutations` detect a link's rotational symmetry (the alpha's four-trefoil cluster has the full A₄). Rendering-free by design, so a separate portrait/compendium library can build on it; consolidates a Gauss-linking integral that was duplicated across three private scripts (the two alpha portraits and a WRT walk-linking analysis). 21 tests (Hopf ±1, unlink 0, orientation/symmetry, K₄ total +6, Borromean pairwise-0 + deletion, Milnor regimes, A₄ symmetry).
- **`condensate.solve_gl_vortex` — gauged Ginzburg-Landau vortex at any GL parameter κ.** Generalizes `solve_bps_vortex` (locked to the BPS self-dual point via first-order shooting) to the full second-order GL / de Vega-Schaposnik system, solved by boundary-value collocation: scalar `f(ρ)` + gauge `a(ρ)` for any `kappa = λ/ξ` and any winding `n`. `kappa = 1` is the BPS point (m_s = m_v) and reproduces `solve_bps_vortex` point-for-point (validated to <5e-3); `kappa < 1` type-I, `kappa > 1` type-II. The collocation solver is also **stable for n ≥ 2**, closing the gap the BPS shooting documented as future work (`solve_bps_vortex` now points n≥2 callers here). Returned as a `GLVortexProfile` (ρ in healing lengths, with `f_at`/`a_at` interpolators and a `supercurrent()` helper for the screening-current sheath `f²(n−a)/ρ`, whose radius migrates outward with κ). Motivated by a carrier form-factor study (where does the framing-charge sit radially? — the gauge-invariant supercurrent sheath of a near-critically-coupled vortex). 7 new tests.
- **Form factors & elastic scattering (`em.form_factor`, `em.mean_square_radius`, `amplitudes.mott_cross_section`, `amplitudes.elastic_form_factor_cross_section`).** Completes the charge-density → form-factor → scattering bridge from `nwt_substrate.em`. `form_factor` returns the spherically-averaged elastic form factor |F(q)| (F(0)=1) of a charge distribution, with a **zero-padded open-BC** option (isolated source — clean low-q, no periodic-image contamination) reusing the Hockney trick; `mean_square_radius` gives ⟨r²⟩ (signed — negative allowed for a net-neutral distribution like the neutron). On the amplitudes side, `mott_cross_section` is the relativistic point-charge dσ/dΩ and `elastic_form_factor_cross_section` applies `dσ/dΩ = (dσ/dΩ)_Mott·|F(q²)|²` (a callable F(q²)), i.e. how a finite charge radius suppresses large-angle scattering. Used to test NWT carrier form factors against the proton dipole / neutron Galster shapes. 4 new tests.

### Changed

- **Structural-constant hygiene + type hints across the shims (code-quality pass, #8).** All structural integers in `condensate.walks` / `condensate.orbit_winding`, `heron.steane_pair_synthesis`, `qpu.decode`, and `em.fields` now source from `isa.constants` (`N_VERTICES_K7`, …) instead of bare literals (AGENTS.md rule 2); the duplicated Hamming(7,4) syndrome bit-weight arithmetic is consolidated into one `_syndrome_to_pos` helper + `_HAMMING_BIT_WEIGHTS` constant (the `3` stays an honest `len(STEANE_STAB_SUPPORTS)`/parity-bit count, **not** aliased to `RANK_SO7`); return/parameter type hints and Parameters/Returns/Raises docstrings were added across `em.fields`, `qft.lagrangians`, `qpu.{decode,runner}` and the Steane helpers; and `fano()` / `bfs_shortest_walks()` gained input-validation guards. Behaviour-preserving. A CodeQL code-scanning workflow was also added to CI.
- **`solitons.faddeev` is the `jax-solitons` engine's reference oracle — now cross-checked live (#9).** The docstrings were corrected: the engine does not *import* these primitives, it is *validated against* them. A cross-engine equivalence gate on the engine side pins this package and asserts the Faddeev energy and Whitehead Hopf charge agree on an identical hopfion field (to ~1e-16), so the two independent implementations can no longer silently drift.
- **`em.maxwell_eh` — convergence guard (PR #2 review follow-up).** The Euler–Heisenberg fixed-point loop now stops early once the relative change in E falls below `tol` (default 1e-5) and issues a `RuntimeWarning` if `n_iter` is exhausted without converging (e.g. `eh_xi` pushed into the non-convergent strong-field regime), rather than silently running a fixed iteration count.
