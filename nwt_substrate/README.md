...
| `nwt.electroweak` | EW / Z resonance | Q.11 | `M_Z`, `Γ_Z`, `sin²θ_W`; `g_V`/`g_A` couplings for SM fermions; full γ + Z + interference σ_total at Z pole = 1985 pb (PDG match 99.2%); `total_width_Z` = 2.487 GeV (PDG 2.495, **99.7%**, with the v0.4.0 hadronic QCD correction); `sin²θ_W = (2+α)/9` is the on-shell angle (<0.1% vs `1−M_W²/M_Z²`) |
