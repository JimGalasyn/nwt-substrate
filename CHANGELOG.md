...
- **`condensate.solve_bps_vortex` — exact BPS vortex profile.** The radial cross-section `f(ρ), a(ρ)` of a winding-`n` abelian-Higgs vortex, by shooting on the first-order Bogomolny equations (n=1 converges to slope c1 ≈ 0.6033). Complements the existing BPS *scalars* in `condensate.abelian_higgs` (healing length, line tension) with the actual profile; returned as a `BPSVortexProfile` with interpolating `f_at`/`a_at`.
